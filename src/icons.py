...
PATHS = {
    ...
    # [FIX] Removed as dead code, confirmed unreferenced across the whole
    # app: "moon", "sun", "check-circle", "alert-circle" and "palette". The
    # light/dark toggle in Configuracoes that "sun"/"moon" were drawn for
    # renders emoji text instead (see SettingsDialog._refresh_theme_mode_button)
    # and never adopted them; status is shown through QLabel#Pill/#StatusDone
    # rather than check/alert glyphs; the theme picker lost its palette
    # button when it moved into Configuracoes.
